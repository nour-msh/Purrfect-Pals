import { View,Text,StyleSheet, FlatList,Modal,Pressable,TextInput,Image,TouchableOpacity} from "react-native";
...
function PetProfile({ route,navigation}){
    ...
    const handleReviewModal = () => setReviewModalVisible(() => !isReviewModalVisible);
    const[ reviewText, setReviewText] = useState("");

     const [reviews, setReviews] = useState([]);
     const {userId,userFullName} = useContext(UserContext)
     const {petId} = route.params.data

     useEffect(()=>{
        axios({
            method: "GET",
            url: `http://192.168.1.3:5000/user/getAllReviewsByPetId/${petId}`,
        }).then((res)=>{
            setReviews(res.data)
        }).catch((error)=>{
            console.warn(error)
    ...
    const handleSubmitReview = () => {
        ...
        axios({
          method: "POST",
          data,
          url: `http://192.168.1.3:5000/user/addReview/${userId}/${petId}`,
        })
          .then((res) => {
              console.log(res);
        ...
        >
            <View style={styles.centeredView}>
                <View style={styles.modalView}>
                    <View>
                    <MaterialIcons onPress={() => setReviewModalVisible(!isReviewModalVisible)} style={styles.close} name="close" size={24} color="black" />
                    </View>
                    <View>
                        <TextInput style={styles.modalText} placeholder='Add Review'
                        value={reviewText} 
